...
(async () => {
  ...
  // JS ile dikey scroll (tek satir tekerlek) — wheel selector 160ms debounce sonrasi sembol degistirmeli
  await p.evaluate(() => { const s = document.querySelector('#wstrip'); s.scrollTop = s.scrollHeight; });
  ...
  const centerOk = await p.evaluate(() => {
    ...
    return Math.abs((b.top + b.height / 2) - (sr.top + sr.height / 2)) < 24;
  });
