...
> **r83 NOTU**: TradingView parite analizi + faz-1 uygulama yapıldı. Tam liste ve "sonraki
> fazlar" için **GAP.md** oku (TV'de olup bizde olmayanların ✅/🔶/⛔/📅 işaretli dökümü).
...
- **r83-TV-PARİTY (GAP.md'e bak)**: TV web+mobil ile birebir parite çalışması faz-1. Yeni: 3m/45m/2h/3h/3M/6M dilimleri; boş mumlar/yüksek-düşük/adımlı/işaretli çizgi/baseline/kolonlar/renko/line-break(3) grafik tipleri; filigran; %ölçek; önceki kapanış çizgisi (ÖK); En Y/D etiketleri; seans kırılım çizgileri; sembol bindir/karşılaştır (⧉ + grafik sağ-tık); fiyat/zaman ekseni sağ-tık menüleri; tarihe git; kısayol penceresi; bar CSV + panoya ekran görüntüsü; arama filtre çipleri + son arananlar; 17 gösterge (83 toplam); gösterge şablonları + grafik düzeni kaydet/yükle; alarm v3 (gösterge koşulu/kesişim, frekans, 1gün/1hafta süresi, geçmiş sekmesi vela.alertLog); replay v2 (⏸/▶, 1-10x hız, geri adım, kontrol çubuğu); sembol bilgi penceresi (satır sağ-tık — /api/quotes'a open/high/low/bid/ask/sector/mcap/pe/eps/div_yield/beta alanları eklendi); mobil ⋯ menüsü (#mobmore, gizli üst çubuk işlevleri); cetvel: gölgeli alan + hacim satırı. Saniye dilimleri köprüde `seconds_not_entitled` (TV'de de ücretli); 2m/4m/90m/2D/3D `custom_resolution` hatası — INT listesine EKLEME. Test: build damgası, 83 IND, renko/filigran/ÖK ekran görüntüsü, alarm paneli sekmeleri, replay oynat/çık, mobil menü 12 öğe — hepsi lokal 3011'de doğrulandı.
- **r82-ALARM-ARM (kullanıcı: "alarm sistemi çalışmıyor, kurulduğu yerde çizgi görünmüyor")**: Kök neden YENİDEN ÜRETİLDİ — `checkAlerts`'te `arm` bayrağı için yer yazılmış ama ARM MEKANİZMASI hiç uygulanmamıştı; alarm güncel fiyata (boş fiyat = son kapanış) kurulunca sonraki İLK WS tick'inde `lp>=fiyat` zaten sağlandığından alarm ANINDA patlıyor + SİLİNİYORDU → kullanıcı çizgiyi hiç göremiyor ("çizgi yok, alarm çalışmıyor"). Düzeltme: `newAlert()` TV crossing semantiği ile `armed` kurar (kuruluş anında koşul sağlanıyorsa armed=false = beklemede; fiyat karşı tarafa dönünce armed=true yazılır, gerçek geçişte çalar). `checkAlerts` armed akışını yönetir; `armed===undefined` eski kayıtlar kuruluş anındaki duruma göre geçici değerlenir (koşul sağlanıyorsa asla anında patlamaz). Alarm çizgisi `paintAlerts` zaten çalışıyordu — silinme durunca görünür. Ek düzeltmeler: (1) `beep()` suspended AudioContext'i resume eder + ilk pointerdown/keydown'da bağlam açılır (jest dışı kurulum ömür boyu sessiz kalıyordu); (2) `refreshQuotes` sonunda + 15 sn'lik interval ile checkAlerts — WS ilk-40 aboneliği dışındaki sembollerin alarmları da çalar; (3) sürükleme kaydı alarm `id` ile eşleşir (fiyat eşitliği kopya alarmlarda yanlış eşleşirdi); (4) beklemede alarm toast'ı "(beklemede …)" içerir. Doğrulama (yerel, canlı tick ile): boş fiyatla Kur → 17 tick kontrolünde alarm duruyor + çizgi çiziliyor; organik geçiş (aşağı in → kur → üstüne çık) → tek `⚡` toast + alarm silindi; armed'sız eski kayıt anında patlamıyor.
- **r81-ZOOM-GORECELI (kullanıcı: "hangi tickerda ne ayar yaptıysak sonrakine geçince de kalmalı — görünen mum, grafiğin konumu")**: r77'nin takvim-zaman zoom kaydı, seans saatleri farklı borsalar arasında (BIST↔ABD, kripto) pencereyi YANLIŞ konuma düşürüyordu — takvim penceresi yeni enstrümanın seansına denk gelmeyince konum ve mum sayısı korunmuyordu. Zoom kaydı artık **GORECELI**: `vela.zoom={periyot:{w,off}}` — `w` = görünen mum aralığı (ekranda kaç mum), `off` = görünen sağ kenar son bardan kaç mum ileride (negatif = son mumun sağında boşluk). Geri yükleme `setVisibleLogicalRange({from: len-1-off-w, to: len-1-off})`. Bu model bar sayısı/seans farkından bağımsızdır — TV'nin sembol geçişinde barSpacing+ofset korumasının aynısı. Eski `{f,t}` kayıtları yükleyicide elenir; ilk geçişte r79 self-seed (saveApplied) baz oluşturur. Periyot-değişim bazi `{w:n+G, off:-(G+1)}` (applyRange'in from=len-n,to=len+G penceresinin tamturani — off=-(G+1), -G DEĞİL, yoksa 1 mum kayma). fit düğmesi artık doğrudan eski format yazmıyor, lastApplied→saveApplied yolunu kullanıyor. DİKKAT: kayıt dinleyicisinin `lastChartTouch>900 → kaydetme` penceresi r74'ten beri duruyor; tek tekerlek adımının range olayı 900ms'den sonra gelirse o anlık görünüm kaydedilmez (fare tekerleğinde pratikte sorun yok).
- **r77-ZOOM-TAKVİM (kullanıcı: "grafik ayarları allak bullak, en sola/en sağa gitmiş")**: r76'daki periyot-bazlı zoom hafızası MANTIKSAL İNDEKS tutuyordu — her hissenin bar sayısı farklı olduğundan (THYAO 1500, yeni hisse 300) mum-numarası restore diğer hissede boşa düşüp bozuk görünüm üretiyordu. Artık görünen aralık TAKVİM ZAMANI olarak kaydedilir: `logicalTimeRange(lr)` mantıksal indeksleri `ct(bar.time)` zamanına çevirir (veri dışı kısım periyot-adımı×sn ile uzatılır), `vela.zoom={periyot:{f,t}}`; geri yükleme `ts.setVisibleRange({from,to})` ile. Eski bozuk format kayıtları yüklemede atılır. LWC kenetlemesi: istenen pencere verinin sonundan ilerideyse pencere son bara kenetlenir (normal). ÖNEMLİ DERS: LWC'de `setVisibleLogicalRange` sonrası `getVisibleLogicalRange` SENKRON okunmaz (bir sonraki karede uygulanır) — `captureZoom`'un gecikmeli okuması bu yüzden ESKI değeri yakalıyordu; okuma yarışından kaçının, uygulanan bilinen değeri yazın (`lastApplied` deseni). **(r81'de takvim modeli kaldırıldı — üstteki r81 maddesi geçerli.)**
